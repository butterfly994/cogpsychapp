...
class App extends React.Component {
    constructor(){
        ...
        this.allGridTypes = [['SimpleSpace', 3, false], ['SimpleSpace', 4, false], 
        ['SimpleSpace', 5, false], ['SimpleSpace', 6, false], 
        ['SimpleSpace', 7, false], ['CloseSpace', 6, false], ['4x4x4', 12, true], 
        ['3x3', 6, false], ['4x4', 8, true], ['4x4', 8, false], ['3x3x3', 9, true]]
        this.partialReport = [['3x3', 6, false, 3], ['4x4', 8, false, 4], ['4x4', 8, true, 4], ['3x3x3', 9, false, 3], ['4x4x4', 12, true, 4]]
    ...
    componentDidMount(){
        ...
        if(isP === 1) {
            ...
            this.setState({isPartial: true, toneDelay: this.toneDelays[toneDelayIdx]})
        }
        else{
            let exposureIdx = Math.floor(Math.random()*this.exposureDurations.length)
            this.setState({exposureDuration: this.exposureDurations[exposureIdx]})
        }
    }
    ...
    render(){
        ...
        if(!this.state.started && !this.state.isPartial){
            ...
            (
                <div className='App'>
                    ...
                    <div className = 'buttonArr'>
                        <div className='button' onClick = {() => this.low.play('durShort')}>Low</div>
                        <div className='button' onClick = {() => this.medium.play('durShort')}>Medium</div>
                        <div className='button' onClick = {() => this.high.play('durShort')}>High</div>
                    </div>
                    <div id='startBtn' onClick = {() => this.setState({started: true, trialInProgress: false})}>Start</div>
                </div>
            )
            ...
            toRender = 
            (
                <Sperling exposureDuration = {this.state.exposureDuration} 
                availableGridTypes = {gridTypes} sets = {5} isPartial = {this.state.isPartial}
                toneDelay = {this.state.toneDelay} low = {this.low} medium = {this.medium} high = {this.high}/>
            )
        }
        return toRender
    }
